...
The native shared library (`liblattice.dylib` / `liblattice.so`) must be available on the system. Install it via the [install script](../getting-started/installation.md) or build from source with `zig build shared`.
