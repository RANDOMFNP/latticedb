# LatticeDB 0.11.0 Release Notes
...
token the lexer never produces; every left-pointing relationship pattern
returned no rows at all; escape sequences in string literals were stored
verbatim; and result columns were named after the wrong thing.

It also makes `db.query()` behave the same way in every binding, running both
reads and writes and choosing its transaction mode from the query, and corrects
`lattice info`, which described the handle's configuration rather than the
database file.
