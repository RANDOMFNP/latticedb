...
This development cycle hardens write ownership, vector persistence, B+Tree
deletion, physical storage reclamation, and the project quality gates. It also
ships durable explicit property equality indexes through the C, Python,
TypeScript, and Go APIs and teaches the Cypher planner to use them for eligible
inline and `WHERE` equality patterns.
...
- `zig build crash-test` and `zig build fuzz` passed.
